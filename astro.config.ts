...
${files
	...
	.map(
		(file) =>
			`\t"${path.sep + path.relative(basePath, file.parentPath) + path.sep + path.parse(file.name).name}": () => import(".${path.sep + path.relative(srcPath, file.parentPath) + path.sep + file.name}?markdown"),`,
	)
