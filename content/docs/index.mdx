...
export function ProjectCard({ name, description, url, docs, logo }) {
  const isTauriSpecta = name === "Tauri Specta"; // TODO: Bruh

  return (
    <div className="flex items-center gap-x-4 px-4 py-4 rounded-lg border bg-fd-card">
      {/* Logo centered within its container */}
      <div className="flex items-center justify-center shrink-0">
        <div className={`relative flex items-center justify-center ${isTauriSpecta ? 'w-20 h-14' : 'w-20 h-20'}`}>
          <img
            src={logo}
            alt={`${name} logo`}
      ...
      {/* Content with proper alignment */}
          <div className={`flex-1 min-w-0${!isTauriSpecta ? ' pl-8' : ''}`}>
      ...
      {/* GitHub link centered within its container */}
      <div className="flex items-center justify-center shrink-0">
        <a
          href={url}
          target="_blank"
