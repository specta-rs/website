...
Known stability issues/bugs:
 ...
 - Avoid creating a namespace if it’s empty (like we did for `Layout::Files`)
 - `Exporter` shouldn’t need to call `typedef_internal`, it should be implicitly done in `export_internal`?
 ...
 - JSDoc issues:
   ...
   - should merge all `@typedef`’s into a single comment
