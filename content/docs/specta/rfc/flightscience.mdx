...
I have got it running on the latest Specta version via Git.

This sprint I am going to focus on getting the PR cleaned up and merged.

I have also been investigating error handling. As TauRPC uses the `specta::specta` macro and parses around `specta::datatype::Function` struct's we already have the result information available which is nice.
