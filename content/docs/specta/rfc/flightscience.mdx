...
Both of these macros where also commonly forgotten when doing a custom `impl Type for T`, so removing them is nice from that perspective. They were commonly implemented incorrectly in Specta's own official crate implementations (😅) which isn't a great sign of a good API.
